<template>
  <Content :loading="loading">
    <template #head>
      <Input
        v-show="$authres(['view_webusermanager_searchgroup'])"
...
export default {
  ...
  methods: {
    select() {
      this.loading = 1;
      this.$request
        .webUserFindByPage(this.page, this.pageSteep, this.selectWorld)
        .then((result) => {
          if (result.flag) {
            this.contexts = result.data.webUsers;
            this.contextSum = result.data.webUserSum;
            this.loading = 2;
          } else {
            this.loading = 3;
          }
